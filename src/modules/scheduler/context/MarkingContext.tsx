...
export const MarkingContext = createContext<MarkingContextProps>(
  {} as MarkingContextProps
);

export const MarkingProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  const [markings, setMarkings] = useState<Marking[]>([]);
  const [floatingTimeRequests, setFloatingTimeRequests] = useState<
    FloatingTimeRequest[]
  >([]);
  ...
  const addMarking = useCallback(
    (marking: Omit<Marking, "id">) => {
      ...
      showNotification(
        "Marcaje agregado",
        "El marcaje ha sido agregado correctamente",
        "success"
      );
    },
    [showNotification]
  ...
  const updateMarking = useCallback(
    (marking: Marking) => {
      setMarkings((prev) =>
        prev.map((m) => (m.id === marking.id ? marking : m))
      );
      showNotification(
        "Marcaje actualizado",
        "El marcaje ha sido actualizado correctamente",
        "success"
      );
    },
    [showNotification]
  ...
  const deleteMarking = useCallback(
    (markingId: string) => {
      ...
      showNotification(
        "Marcaje eliminado",
        "El marcaje ha sido eliminado correctamente",
        "success"
      );
    },
    [showNotification]
  ...
  const approveFloatingTime = useCallback(
    (requestId: string) => {
      ...
      showNotification(
        "Solicitud aprobada",
        "La solicitud de tiempo flotante ha sido aprobada",
        "success"
      );
    },
    [showNotification]
  ...
  const rejectFloatingTime = useCallback(
    (requestId: string) => {
      ...
      showNotification(
        "Solicitud rechazada",
        "La solicitud de tiempo flotante ha sido rechazada",
        "success"
      );
    },
    [showNotification]
  ...
  const calculateWorkedHours = useCallback(
    (employeeId: string, date: Date) => {
      const employeeMarkings = getMarkingsByEmployeeId(employeeId).filter(
        (marking) => {
          const markingDate = new Date(marking.time);
          return (
            markingDate.getFullYear() === date.getFullYear() &&
